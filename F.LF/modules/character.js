...
define(['F.LF/modules/livingobject','F.LF/modules/global','F.LF/core/combodec','F.LF/core/util','F.LF/modules/util', 'F.LF/modules/AI'],
function(livingobject, Global, Fcombodec, Futil, util, AI)
{
	...
	var states=
	{
			...
			case 'combo':
				...
				default:
					...
					if ($.rudolf_state.is_copied && tag==='hit_ja') {
						// Transform back to Rudolf / SBF
						var uid = $.rudolf_state.uid;
						var oid = $.rudolf_state.original_oid;
						var char = newCharFrom($, oid);
						$.match.character[uid] = char;
						char.trans.frame(245);

						// Transform clones
						var clones = util.select_from($.match.character, function (_char) { return _char.rudolf_state.is_cloned && _char.rudolf_state.master_uid == $.rudolf_state.uid })
						...
						for (c in clones) {
							...
							var cchar = newCharFrom(clone, oid);
		...
		{	var $=this;
			...
			case 'combo':
				...
				case 'att':
					...
					//
					var hitVol;
					if ($.data.frame[72] && $.data.frame[72].itr) {
						hitVol = $.data.frame[72].itr
					} else if ($.data.frame[73] && $.data.frame[73].itr) {
						hitVol = $.data.frame[73].itr
					} else if ($.data.frame[71] && $.data.frame[71].itr) {
						hitVol = $.data.frame[71].itr
					} else if ($.data.frame[70]) {
						hitVol = { x: 40, y: 5, w: 35, h: 45 };
					}
					var vol=$.mech.volume(Futil.make_array(hitVol)[0]); //super punch frames
		...
		'9995': function(event, K)
		{
			...
			switch (event) {
				case 'frame':
					...
					var transform_id = ($.id === 6)?50:61 // LouisEX or CZ Bruce Lee
					var player_obj = util.select_from($.match.data.object,{ id: transform_id });
					var pdata = player_obj.data;
					$.destroy();
					var char = new character(char_config, pdata, transform_id);
		...
		'9996': function(event, K)
		{
			...
			switch (event) {
			case 'frame':
				var oid_1 = ($.id === 6)?217:250 // LouisEX armur or CZ Bruce Lee armur
				var oid_2 = ($.id === 6)?218:251 // LouisEX armur or CZ Bruce Lee armur
				$.match.create_object({
					kind: 1, x: 5, y: 3, action: 399, dvx: 10, dvy: -5, oid: oid_1, facing: 0
				}, $);
				$.match.create_object({
					kind: 1, x: 5, y: 5, action: 399, dvx: 7, dvy: -5, oid: oid_1, facing: 1
				}, $);
				$.match.create_object({
					kind: 1, x: 3, y: 10, action: 399, dvx: 10, dvy: -10, oid: oid_1, facing: 0
				}, $);
				$.match.create_object({
					kind: 1, x: -15, y: 15, action: 399, dvx: 8, dvy: -8, oid: oid_1, facing: 1
				}, $);
				$.match.create_object({
					kind: 1, x: 0, y: 0, action: 399, dvx: 0, dvy: -5, oid: oid_2, facing: 0
				}, $);
				$.effect.super = true;
			break;
		}},
	...
	var idupdates = //nasty fix (es)
	{
				...
				case 'generic_combo':
					if( tag==='hit_ja') { //Transform
						...
						if ($.rudolf_state.copied_oid) {
							...
							var clones = util.select_from($.match.character, function (char) { return char.rudolf_state.is_cloned && char.rudolf_state.master_uid == $.rudolf_state.uid })
							if (typeof clones == "object" && !(clones instanceof Array))
								clones = [clones]
		...
		'64': function(event,K,tag) //SBF (same as Rudolf)
		{
			var $=this;
			switch (event)
			{
		...
		'70': function(event,K,tag) //Mai (same as Rudolf)
		{
			var $=this;
			switch (event)
			{
		...
		'71': function(event,K,tag) //Sushi (same as Rudolf)
		{
			var $=this;
			switch (event)
			{
	...
	function newCharFrom(oldChar, oid) {
		...
		if (oid == 5 || oid == 64 || oid == 70 || oid == 71) { // Transform back to Rudolf / SBF / Mai / Sushi
	...
	function character(config,data,thisID)
	{
		...
		$.rudolf_state = { // only for Rudolf, SBF, Mai
			clones_uid: [], // Clones
