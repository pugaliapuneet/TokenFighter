...
define(['F.LF/core/util','F.LF/core/controller','F.LF/modules/sprite-select',
'F.LF/modules/network','F.LF/modules/factories','F.LF/modules/scene','F.LF/modules/background','F.LF/modules/AI','F.LF/third_party/random','F.LF/modules/util',
'F.LF/modules/global'],
	...
	match.prototype.process_task=function(T)
	{
		...
		case 'create_object':
			if( T.opoint.oid)
			{
				...
				if (OBJ.type == "character") { // Rudolf, SBF, Mai, Sushi clone
					if (T.opoint.oid === 5 || T.opoint.oid === 64 || T.opoint.oid === 70 || T.opoint.oid === 71 ) {
						...
						obj.rudolf_state.original_oid = T.opoint.oid;
						T.parent.rudolf_state.clones_uid.push(obj.uid);
					} else if (T.opoint.oid === 52 || T.opoint.oid === 62) { // Julian or Arthur shadow
						config.controller = new AI.controller();
						var obj = new factory[OBJ.type](config, OBJ.data, T.opoint.oid);
		...
		if( $.gameover_state)
		{
			...
			for( var i=0; i<$.panel.length; i++)
			{
				if( $.panel[i].uid!==undefined)
				{
					...
					// Read rudolf_state to check the character is Rudolf/SBF (oid: 5/64)
					var is_rudolf_or_sbf = !!ch.rudolf_state.uid;
					var path = is_rudolf_or_sbf ? util.select_from($.data.object, { id: ch.rudolf_state.original_oid }).data.bmp.small : ch.data.bmp.small
