define(['F.LF/modules/global','F.LF/modules/network','F.LF/modules/soundpack','F.LF/modules/match','F.LF/modules/util','F.LF/modules/touchcontroller','F.LF/third_party/random',
'F.LF/core/util','F.LF/modules/sprite-select','F.LF/core/sprite-dom','F.LF/core/animator','F.LF/core/controller','F.LF/core/resourcemap','F.LF/core/support'],
...
function Manager(package, buildinfo, tokens)
{
	var limitedTokens = ["Mai"]
	...
	this.create=function()
	{
		...
		util.div('maximize_button').onclick=function()
		{
			if( Fsupport.css2dtransform)
			{
				...
				if( !window_state.maximized || (fullscreen && !fullscreen_state))
				{
					...
					this.firstChild.innerHTML='Resize';
		...
		// Filter
		player_char_list =  char_list.filter(function (char) { return tokens.indexOf(char.name) >= 0 })

		char_list[-1] = {name:'Random'}
		player_char_list[-1] = {name:'Random'}
		...
		name_list = Futil.extract_array(char_list, 'name').name;
		name_list[-1] = "Random"
		...
		// Remove limited edition NFT after setup
		char_list = char_list.filter(function (char) { return limitedTokens.indexOf(char.name) < 0 })
		char_list[-1] = {name:'Random'}
				...
				this.dialog = new vertical_menu_dialog({
					...
					onclick: function(I)
					{
						...
						else if( I===1)
						{
							...
							manager.alert('Network game is coming soon!');
						}
						else if( I===2)
						{
			...
			create:function()
			{
					...
					key: function(i,key)
					{
						switch(key)
						{
							...
							case 'right':
								...
								{
									players[i].selected++;
									if( players[i].selected>=player_char_list.length)
							...
							case 'left':
								if( players[i].step===1)
								{
									...
									if( players[i].selected<-1)
										players[i].selected = player_char_list.length-1;
								}
								if( players[i].step===2)
								{
									players[i].team--;
						...
						for( var i=0; i<players.length; i++)
						{
							switch (players[i].step)
							{
								...
								case 1:
									...
									players[i].textbox[1].innerHTML = player_char_list[players[i].selected].name;
									players[i].textbox[2].innerHTML = '';
									players[i].ani.rewind();
									// players[i].sp.switch_img(players[i].selected);
									players[i].sp.switch_img(player_char_list[players[i].selected].name);
								break;
								case 2:
						...
						for( var i=0; i<players.length; i++)
						{
							switch (players[i].step)
							{
								case 0:
									...
									players[i].ani.rewind();
									// players[i].sp.switch_img(players[i].selected);
									players[i].sp.switch_img(char_list[players[i].selected].name);
								break;
								case 1:
									players[i].textbox[0].style.color = static_color(i);
									players[i].textbox[1].innerHTML = char_list[players[i].selected].name;
				...
				{	//step 3
					...
					update_random: function()
					{
						...
						{
							var len = (players[i].type === "human")?player_char_list.length:char_list.length;
							var list = (players[i].type === "human")?player_char_list:char_list;
							players[i].selected = Math.floor(randomseed.next()*len);
							players[i].textbox[1].innerHTML = list[players[i].selected].name;
							// players[i].sp.switch_img(players[i].selected);
							players[i].sp.switch_img(list[players[i].selected].name);
						}
			...
			for( var i=0; i<players.length; i++)
			{
				if( players[i].use)
					arr.push({
						...
						id: players[i].type==='human' ? player_char_list[players[i].selected].id : char_list[players[i].selected].id,
